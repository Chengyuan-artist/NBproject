...
new_sheet = databook.add_sheet('0')
...
#print(type(data))
# 遍历json里的新闻
Titleset=['file_id', 'news_id', 'news_title', 'news_content', 'sentiment_1', 'sentiment_2']

for i in range(len(Titleset)):
    new_sheet.write(0, i, Titleset[i])
''''
for i in range(len(data)-1, -1, -1):
     if data[i]['news_emotion'][0] == 'objective':
         data.pop(i)
'''
for index, news in enumerate(data):
    # print(news)
    # 给指定行、指定列写入指定内容
    new_sheet.write(index+1, 0, index+1)
    new_sheet.write(index+1, 1, news['id'])
    new_sheet.write(index+1, 2, news['news_title_translate_en'])
    if 'news_content_translate_en' in news:
        # print(news['news_content_translate_en'])
        new_sheet.write(index+1, 3, news['news_content_translate_en'])
    else:
        new_sheet.write(index + 1, 3, 'aaa')
    new_sheet.write(index+1, 4, news['news_emotion'][0])
    # 判断是否有第二个情感
    if len(news['news_emotion']) > 1:
        new_sheet.write(index+1, 5, news['news_emotion'][1])
    else:
        new_sheet.write(index+1, 5, 'none')

databook.save('test.xls')
...
for category in sentiment_vocab:
    ...
    #print(result)       # !!!第一个数代表不是的概率，第二个代表是的概率